...
interface PlatformImporter {
  name: string
  importPhotos: (connectionData: any, clientId: string) => Promise<ImportResult>
}
...
class PixiesetImporter implements PlatformImporter {
  ...
  async importPhotos(connectionData: any, clientId: string): Promise<ImportResult> {
    try {
...
async function storeImportedData(clientId: string, platform: string, galleries: any[]) {
