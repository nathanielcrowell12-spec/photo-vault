...
import { Upload, X, FileArchive, CheckCircle, AlertCircle } from 'lucide-react'
import { useAuth } from '@/contexts/AuthContext'

interface FastZipUploadModalProps {
  platform?: string
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
...
export default function FastZipUploadModal({
  platform = 'ZIP Upload',
  isOpen,
  onClose,
  onSuccess
}: FastZipUploadModalProps) {
  const { user } = useAuth()
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await processZipFile(galleryId, path)
  ...
  const processZipFile = async (galleryId: string, storagePath: string): Promise<void> => {
    return new Promise((resolve, reject) => {
      const eventSource = new EventSource(`/api/v1/upload/process`, {
        // Note: EventSource doesn't support POST directly, so we'll need to modify this
      })

      // We'll need to modify the processing endpoint to support GET with query params
      // For now, let's use fetch with streaming
      fetch('/api/v1/upload/process', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Import from {platform} - Fast Upload
          </DialogTitle>
        </DialogHeader>
