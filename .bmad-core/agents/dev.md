...
commands:
  ...
  - review-code {target} [--focus]: |
      Comprehensive code review using code-review task.
      Target can be: file, directory, "diff" (staged), "diff --unstaged", "diff HEAD~1", or "pr:{number}"
      Optional --focus: security | performance | architecture | testing | all
      Example: *review-code src/services/ --focus security
